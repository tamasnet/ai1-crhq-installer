...
| **D-22** | **Skills register as org + `locked` by default** (`skill_type:'org'`, `locked:true`). Override per-skill via the package-manifest component entry `install_type: user` (→ `skill_type:'user'`, unlocked — the prior behavior), or globally via `--install-skills-as-user` (forces **all** skills to user; **wins** over a per-skill `install_type`). Value must be `org`\|`user` (else `ManifestError`). Assets still install to `INSTALL_BASE_DIR`. | **Confirmed (user, 2026-06-11)** | New default per user: org skills are the norm; user skills are the opt-out. We lack write access to where real org skills live, so **only the DB registration changes** — the filesystem payload stays under `INSTALL_BASE_DIR`. `install_type` lives in the **package manifest entry** (per user), not SKILL.md. `upsertSkill` now writes `locked` and, on an actual update of a locked row, unlocks first (the live PG trigger forbids UPDATE on locked rows; C5); idempotent re-runs of an unchanged org skill make no writes and leave it locked. Removal unlocks-then-deletes (or `--respect-locks` skips). |
