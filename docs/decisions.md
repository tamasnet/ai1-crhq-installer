...
| **D-24** | **Install log** at `${PACKAGES_DIR}/install.json` (default `~/packages`): per-package, per-component `{type, name, version?, installed_at, source}` (source = the component's manifest file relative to the package root). Never written in dry-run/status; uninstall deletes entries outright (package key removed with its last component); `--sandbox` redirects `PACKAGES_DIR` to a throwaway dir; a write failure warns but never fails the install | A queryable record of what's installed, independent of the DB. Removal-not-tombstoning keeps it a statement of current state, not a history (git/DB hold history). |
