...
- Skill: `skills/<key>.md` with frontmatter `name`, `version`, `description`; body becomes `skills.content`; optional `skills/<key>/` assets are copied to `SKILLS_BASE_DIR/<key>`.
- Recipe: `recipes/<name>.md` with frontmatter `name`, `description`, optional `version`; body becomes `recipes.content`.
- Agent: `agents/<key>.md` with frontmatter `name`, `display_name`, optional config/links/version; body becomes `agents.instructions`; optional `agents/<key>/` files copy to `AGENT_BRAINS_DIR/<key>`.
- Job: `jobs/<name>.yaml` with `name`, `schedule`, `script`; scripts resolve under `SKILLS_BASE_DIR`.
- Service: `services/<name>/service.yaml` with `name`, `version`, `start`; optional `app_port`, `app_deploy` (`default` \| `none` \| `nginx` \| `pm2`); source copies to `$SERVICES_BASE_DIR/<name>`, with `.env` and (per `app_deploy`) PM2 and/or nginx generated by the installer. `port` is deprecated — use `app_port`.
- Project: `projects/<name>/project.yaml` with the same fields as a service; source symlinks to `/opt/projects/user/<name>` by default (or copies with `--copy-projects`), with `.env` and (per `app_deploy`) PM2 and/or nginx generated by the installer.
