...
advisory actions in order. Supported action types:
  ...
  diff-package    compare a package against live state and return the JSON report as action result data

Options:
  --limit=<n>   maximum number of actions to process (default: all; n may be 0)
  --dry-run     validate and report actions without performing them or updating actions.json
...
function parseArgs(argv) {
  const flags = { json: false, dryRun: false, limit: null };
  for (const token of argv) {
    ...
    if (name === '--json') {
      ...
      if (hasEq) throw new UsageError('option --dry-run does not take a value');
      flags.dryRun = true;
    } else if (name === '--limit') {
...
async function main() {
  ...
  const result = await runActions(
    ...
    { now: new Date(), log: flags.json ? undefined : log },
  );

  if (flags.json) {
    console.log(JSON.stringify(result, null, 2));
  } else if (result.dryRun) {
    log.info(`[dry-run] ${result.wouldProcess} action${result.wouldProcess === 1 ? '' : 's'} would be processed`);
    log.info(`${result.remaining} action${result.remaining === 1 ? '' : 's'} remain in ${result.dest}`);
