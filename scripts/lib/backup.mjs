...
// writes under BACKUP_BASE. No dry-run, no sandbox, no locks. Services are out of scope in v1
// (not DB-resident; their source of truth is the original package).
//
...
export async function runBackup(ctx, { now = new Date() } = {}) {
  ...
  try {
    ...
    writeIfChanged(join(staging, 'ai1-package.yaml'), dumpYaml(meta), { dryRun: false });
    ...
    ctx.reportExtra = { package: { name: pkgName, version: meta.version, dir: destDir } };
